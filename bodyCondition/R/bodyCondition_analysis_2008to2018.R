...
# Years 2008to2018
mergedClean2008to2018 <-
  mergedClean[mergedClean$yearOnly > 2007 &
                mergedClean$yearOnly < 2019,]

# Select the subset of variables to be used in analysis
subset_mergedClean2008to2018 <-
  mergedClean2008to2018[, c(
    "uniqueName",
    "bodyScore",
    "yearOnly",
    "monthOnly",
...
str(subset_mergedClean2008to2018)

subset_mergedClean2008to2018 <-
  subset_mergedClean2008to2018[complete.cases(subset_mergedClean2008to2018),]

head(subset_mergedClean2008to2018)

# Centre and scale continuous variables
subset_mergedClean2008to2018$yearOnlyScale <-
  as.numeric(scale(subset_mergedClean2008to2018$yearOnly))
subset_mergedClean2008to2018$monthOnlyScale <-
  as.numeric(scale(subset_mergedClean2008to2018$monthOnly))

# Get only scaled variables
subset_mergedClean2008to2018 <- subset_mergedClean2008to2018[, c(
  "bodyScore",
  "uniqueName",
  "yearOnlyScale",
  "monthOnly",
...
subset_mergedClean2008to2018 <-
  subset_mergedClean2008to2018[complete.cases(subset_mergedClean2008to2018), ]
subset_mergedClean2008to2018 <-
  droplevels(subset_mergedClean2008to2018)
subset_mergedClean2008to2018$monthOnly <-
  as.factor(subset_mergedClean2008to2018$monthOnly)
summary(subset_mergedClean2008to2018)

# Some diagnostics of effort
table(subset_mergedClean2008to2018$event,
      subset_mergedClean2008to2018$scorerCode)
table(
  subset_mergedClean2008to2018$uniqueName,
  subset_mergedClean2008to2018$scorerCode
)
...
names(subset_mergedClean2008to2018)

pairs(
  subset_mergedClean2008to2018[, c(1, 3, 5, 13)],
  panel = panel.lm,
  cex = 1,
  pch = 19,
  col = adjustcolor(4, .4),
...
subset_mergedClean2008to2018$row <-
  1:nrow(subset_mergedClean2008to2018)

subset_mergedClean2008to2018avEvent <-
  aggregate(bodyScore ~ event, FUN = mean, data = subset_mergedClean2008to2018)
hist(subset_mergedClean2008to2018avEvent$bodyScore)
summary(subset_mergedClean2008to2018avEvent)

avEventDat <-
  merge(x = subset_mergedClean2008to2018avEvent,
        y = subset_mergedClean2008to2018[, c("yearOnlyScale", "monthOnly", "event")],
        by = "event",
        all.y = F)
...
pivotCounts <-
  count(subset_mergedClean2008to2018, event, age) %>% pivot_wider(values_from = n, names_from = age)
pivotCounts$newAge <-
...
lmerList <- list(
  ...
  mod1_lmer <-
    lmer(
      ...
      REML = FALSE,
      data = subset_mergedClean2008to2018,
...
par(mfrow = c(2, 3))
plot(resid(lmerList[[topLmer]]) ~ subset_mergedClean2008to2018$yearOnlyScale)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmerList[[topLmer]]) ~ subset_mergedClean2008to2018$monthOnly)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmerList[[topLmer]]) ~ as.factor(subset_mergedClean2008to2018$age))
abline(h = 0, col = "red", lty = 2)
plot(resid(lmerList[[topLmer]]) ~ subset_mergedClean2008to2018$scorerCode)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmerList[[topLmer]]) ~ as.factor(subset_mergedClean2008to2018$event))
abline(h = 0, col = "red", lty = 2)
...
lmer1AICc$timePeriod = "2008 to 2018"
...
write.csv(lmer1AICc, "../results/2008to2018/AICc_lmer1_2008to2018.csv", row.names = FALSE)
...
par(mfrow = c(2, 2))
plot(resid(lmer1List[[topLmer1]]) ~ subset_mergedClean2008to2018$yearOnlyScale)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmer1List[[topLmer1]]) ~ subset_mergedClean2008to2018$age)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmer1List[[topLmer1]]) ~ subset_mergedClean2008to2018$monthOnly)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmer1List[[topLmer1]]) ~ subset_mergedClean2008to2018$scorerCode)
abline(h = 0, col = "red", lty = 2)
...
annualCoefs_top_glm$lowerCI <-
  as.numeric(annualChange_confint_top_glm [1:13, ][, 1])
annualCoefs_top_glm$upperCI <-
  as.numeric(annualChange_confint_top_glm [1:13, ][, 2])
write.csv(
  annualCoefs_top_glm,
  "../results/2008to2018/coefs_annualChange_top_glm_2008to2018.csv"
)

#  Top lmer
annualChange_confint_top_lmer <-
  confint.merMod(
    topModList2008to2018[[2]],
    level = 0.95,
    method = "boot",
    nsim = nSim
  )
...
annualCoefs_top_lmer <-
  data.frame(coefTable(topModList2008to2018[[2]]))
...
write.csv(
  annualCoefs_top_lmer,
  "../results/2008to2018/coefs_annualChange_top_lmer_2008to2018.csv"
)

#  Top lmer1
annualChange_confint_top_lmer1 <-
  confint.merMod(
    topModList2008to2018[[3]],
    level = 0.95,
    method = "boot",
    nsim = nSim
  )
...
annualCoefs_top_lmer1 <-
  data.frame(coefTable(topModList2008to2018[[3]]))
...
write.csv(
  annualCoefs_top_lmer1,
  "../results/2008to2018/coefs_annualChange_top_lmer1_2008to2018.csv"
)

# Save everything
save(list = ls(.GlobalEnv),
     file = "../results/2008to2018/bodyCondition_2008to2018.Rdata")
