...
# Years 1997to2007
mergedClean1997to2007 <-
  mergedClean[mergedClean$yearOnly > 1996 &
                mergedClean$yearOnly < 2008,]

# Select the subset of variables to be used in analysis
subset_mergedClean1997to2007 <-
  mergedClean1997to2007[, c(
    "uniqueName",
    "bodyScore",
    "yearOnly",
    "monthOnly",
...
str(subset_mergedClean1997to2007)

subset_mergedClean1997to2007 <-
  subset_mergedClean1997to2007[complete.cases(subset_mergedClean1997to2007),]

head(subset_mergedClean1997to2007)

# Centre and scale continuous variables
subset_mergedClean1997to2007$yearOnlyScale <-
  as.numeric(scale(subset_mergedClean1997to2007$yearOnly))
subset_mergedClean1997to2007$monthOnlyScale <-
  as.numeric(scale(subset_mergedClean1997to2007$monthOnly))

# Get only scaled variables
subset_mergedClean1997to2007 <- subset_mergedClean1997to2007[, c(
  "bodyScore",
  "uniqueName",
  "yearOnlyScale",
  "monthOnly",
...
subset_mergedClean1997to2007 <-
  subset_mergedClean1997to2007[complete.cases(subset_mergedClean1997to2007), ]
subset_mergedClean1997to2007 <-
  droplevels(subset_mergedClean1997to2007)
subset_mergedClean1997to2007$monthOnly <-
  as.factor(subset_mergedClean1997to2007$monthOnly)
summary(subset_mergedClean1997to2007)

# Some diagnostics of effort
table(subset_mergedClean1997to2007$event,
      subset_mergedClean1997to2007$scorerCode)
table(
  subset_mergedClean1997to2007$uniqueName,
  subset_mergedClean1997to2007$scorerCode
)
...
names(subset_mergedClean1997to2007)

pairs(
  subset_mergedClean1997to2007[, c(1, 3, 5, 13)],
  panel = panel.lm,
  cex = 1,
  pch = 19,
  col = adjustcolor(4, .4),
...
# Unique 'events' (i.e. series of images that are close together in time, mostly of same animal but not always) were created to deal with pseudoreplication, but still some question if correct to include these as a random effect in a LMM. Try multiple approaches

# 1. Average bodyScore for each 'event', ignoring scorer effects, simple GLM
subset_mergedClean1997to2007$row <-
  1:nrow(subset_mergedClean1997to2007)

subset_mergedClean1997to2007avEvent <-
  aggregate(bodyScore ~ event, FUN = mean, data = subset_mergedClean1997to2007)
hist(subset_mergedClean1997to2007avEvent$bodyScore)
summary(subset_mergedClean1997to2007avEvent)

avEventDat <-
  merge(x = subset_mergedClean1997to2007avEvent,
        y = subset_mergedClean1997to2007[, c("yearOnlyScale", "monthOnly", "event")],
        by = "event",
        all.y = F)
...
pivotCounts <-
  count(subset_mergedClean1997to2007, event, age) %>% pivot_wider(values_from = n, names_from = age)
pivotCounts$newAge <-
...
lmerList <- list(
  ...
  mod1_lmer <-
    lmer(
      ...
      REML = FALSE,
      data = subset_mergedClean1997to2007,
...
par(mfrow = c(2, 3))
plot(resid(lmerList[[topLmer]]) ~ subset_mergedClean1997to2007$yearOnlyScale)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmerList[[topLmer]]) ~ subset_mergedClean1997to2007$monthOnly)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmerList[[topLmer]]) ~ as.factor(subset_mergedClean1997to2007$age))
abline(h = 0, col = "red", lty = 2)
plot(resid(lmerList[[topLmer]]) ~ subset_mergedClean1997to2007$scorerCode)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmerList[[topLmer]]) ~ as.factor(subset_mergedClean1997to2007$event))
abline(h = 0, col = "red", lty = 2)
...
lmer1AICc$timePeriod = "1997 to 2007"
...
write.csv(lmer1AICc, "../results/1997to2007/AICc_lmer1_1997to2007.csv", row.names = FALSE)
...
par(mfrow = c(2, 2))
plot(resid(lmer1List[[topLmer1]]) ~ subset_mergedClean1997to2007$yearOnlyScale)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmer1List[[topLmer1]]) ~ subset_mergedClean1997to2007$age)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmer1List[[topLmer1]]) ~ subset_mergedClean1997to2007$monthOnly)
abline(h = 0, col = "red", lty = 2)
plot(resid(lmer1List[[topLmer1]]) ~ subset_mergedClean1997to2007$scorerCode)
abline(h = 0, col = "red", lty = 2)
...
annualCoefs_top_glm$lowerCI <-
  as.numeric(annualChange_confint_top_glm [1:11, ][, 1])
annualCoefs_top_glm$upperCI <-
  as.numeric(annualChange_confint_top_glm [1:11, ][, 2])
write.csv(
  annualCoefs_top_glm,
  "../results/1997to2007/coefs_annualChange_top_glm_1997to2007.csv"
)

#  Top lmer
annualChange_confint_top_lmer <-
  confint.merMod(
    topModList1997to2007[[2]],
    level = 0.95,
    method = "boot",
    nsim = nSim
  )
...
annualCoefs_top_lmer <-
  data.frame(coefTable(topModList1997to2007[[2]]))
annualCoefs_top_lmer$lowerCI <-
  as.numeric(annualChange_confint_top_lmer [4:18, ][, 1])
annualCoefs_top_lmer$upperCI <-
  as.numeric(annualChange_confint_top_lmer [4:18, ][, 2])
write.csv(
  annualCoefs_top_lmer,
  "../results/1997to2007/coefs_annualChange_top_lmer_1997to2007.csv"
)

#  Top lmer1
annualChange_confint_top_lmer1 <-
  confint.merMod(
    topModList1997to2007[[3]],
    level = 0.95,
    method = "boot",
    nsim = nSim
  )
...
annualCoefs_top_lmer1 <-
  data.frame(coefTable(topModList1997to2007[[3]]))
...
write.csv(
  annualCoefs_top_lmer1,
  "../results/1997to2007/coefs_annualChange_top_lmer1_1997to2007.csv"
)

# Save everything
save(list = ls(.GlobalEnv),
     file = "../results/1997to2007/bodyCondition_1997to2007.Rdata")
